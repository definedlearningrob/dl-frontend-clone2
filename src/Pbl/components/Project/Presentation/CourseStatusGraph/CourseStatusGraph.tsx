...
export const CourseStatusGraph = ({ answer, index, questionsLength }: Props) => {
  ...
  const icon = useMemo(() => {
    const iconClasses = styles[status.toLowerCase()];

    return {
      [STUDENT_GRADING_STATUS.NOT_STARTED]: (
        <SharedIcon className={iconClasses} icon={<NotStartedIcon />} size='xs' />
      ),
      [STUDENT_GRADING_STATUS.SUBMITTED]: (
        <SharedIcon className={iconClasses} icon={<SubmittedIcon />} size='xs' />
      ),
      [SUBMISSION_GRADE_STATUS.ACCEPTED]: (
        <SharedIcon className={iconClasses} icon={<AcceptedIcon />} size='xs' />
      ),
      [SUBMISSION_GRADE_STATUS.NOT_ACCEPTED]: (
        <SharedIcon className={iconClasses} icon={<RejectedIcon />} size='xs' />
      ),
  ...
  return (
    <div className='relative flex items-center justify-center'>
      ...
      {index + 1 < questionsLength && <span className={styles.statusDivider} />}
    </div>
  );
};
