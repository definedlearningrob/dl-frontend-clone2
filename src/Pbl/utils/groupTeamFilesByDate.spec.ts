...
import { groupFilesByDate } from './groupTeamFilesByDate';
...
describe('groupTeamFilesByDate', () => {
  ...
  it('should group the files when they are from the same user and within 300 seconds(5min)', () => {
    ...
    expect(groupedFiles[0].content).toHaveLength(1); // within time but user 1
    expect(groupedFiles[1].content).toHaveLength(1); // within time but user 2
    expect(groupedFiles[2].content).toHaveLength(3); // all files are random within 300 seconds user 3
    expect(groupedFiles[3].content).toHaveLength(1); // more than 300 seconds diff user 3
    expect(groupedFiles[4].content).toHaveLength(2); // 300 seconds diff user 4
  });
});
