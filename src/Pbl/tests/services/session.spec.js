...
describe('services | session', () => {
  ...
  it('creates proper login request when calling with user type', () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      `${process.env.VITE_DL_API_HOST}/api/learning/v1/users/sign_in`,
      {
        body: JSON.stringify({
          user: {
            username: 'somelogin',
  ...
  it('creates proper login request when calling with student type and sets token based on response', async () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(fetch).toHaveBeenCalledWith(
      `${process.env.VITE_DL_API_HOST}/api/learning/v1/students/sign_in`,
      {
        body: JSON.stringify({
          student: {
            username: 'somelogin',
  ...
  it('creates proper authorize request when calling with user type and sets token based on response', async () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(fetch).toHaveBeenCalledWith(
      `${process.env.VITE_DL_API_HOST}/api/learning/v1/users/oauth2_callback`,
      {
        body: JSON.stringify({
  ...
  it('creates proper authorize request when calling with student type and sets token based on response', async () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(fetch).toHaveBeenCalledWith(
      `${process.env.VITE_DL_API_HOST}/api/learning/v1/students/oauth2_callback`,
      {
        body: JSON.stringify({
          //eslint-disable-next-line camelcase
          auth_code: 'somecode',
  ...
  it('set proper url based on type and provider', () => {
    ...
    expect(window.location.href).toEqual(`${process.env.VITE_PBL_GOOGLE_AUTH}/student`);

    login({ provider: 'google', type: 'user' });

    expect(window.location.href).toEqual(`${process.env.VITE_PBL_GOOGLE_AUTH}/user`);

    login({ provider: 'clever', type: 'student' });

    expect(window.location.href).toEqual(`${process.env.VITE_PBL_CLEVER_AUTH}/student`);

    login({ provider: 'clever', type: 'user' });

    expect(window.location.href).toEqual(`${process.env.VITE_PBL_CLEVER_AUTH}/user`);

    login({ provider: 'classLink', type: 'student' });

    expect(window.location.href).toEqual(`${process.env.VITE_PBL_CLASSLINK_AUTH}/student`);

    login({ provider: 'classLink', type: 'user' });

    expect(window.location.href).toEqual(`${process.env.VITE_PBL_CLASSLINK_AUTH}/user`);
  });
  ...
  it('calls DELETE on logout and clears token', () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(fetch).toHaveBeenCalledWith(
      `${process.env.VITE_DL_API_HOST}/api/learning/v1/users/sign_out`,
      {
        method: 'DELETE',
        headers: {
          Accept: 'application/json',
