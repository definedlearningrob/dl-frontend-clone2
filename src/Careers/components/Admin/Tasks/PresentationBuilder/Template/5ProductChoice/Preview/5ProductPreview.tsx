...
import Image from './5ProductPreviewImage';
...
function FiveProductPreview({ slide: { content }, slides }: Props) {
  const [
    mainTitle,
  ...
  const indexOfFifthLinkSlide = flattenSlides?.findIndex(
    (slide) => slide.id === fifthLink?.targetId
  );
  ...
  return (
    <main>
      ...
      <div className={styles.flex}>
        ...
        {fifthLink && isValidIndex(indexOfFifthLinkSlide) && (
          <a className={styles.btnWrapper} href={`#/${indexOfFifthLinkSlide}`}>
            <SharedButton variant='primary'>{fifthLink.text}</SharedButton>
          </a>
        )}
      </div>
    </main>
...
export default FiveProductPreview;
