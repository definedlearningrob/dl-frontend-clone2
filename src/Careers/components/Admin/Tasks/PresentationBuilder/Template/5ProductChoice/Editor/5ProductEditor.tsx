...
import Image from './5ProductEditorImage';
...
function FiveProductEditor({ slide: { content }, handleSelectSlideContent }: Props) {
  ...
  const product5TextWrapper = useRef<HTMLDivElement | null>(null);
  ...
  const handleOverflowStateChange =
    (id: '7' | '8' | '9' | '10' | '11') => (overflowing: boolean) => {
  ...
  return (
    <main className={styles.products}>
        ...
        <div className={styles.contentWrapper}>
          <div ref={product4TextWrapper} className={styles.description}>
            <TextItem
              ...
              onOverflowChange={handleOverflowStateChange('10')}
            />
          </div>
        </div>
        {fourthLink && (
      ...
      <div className={styles.flex}>
        <Image handleSelectSlideContent={handleSelectSlideContent} imageContentId='5' />
        <div className={styles.contentWrapper}>
          <div ref={product5TextWrapper} className={styles.description}>
            <TextItem
              className={textClasses}
              handleSelectSlideContent={handleSelectSlideContent}
              item={fifthProductDescription}
              wrapper={product5TextWrapper}
              onOverflowChange={handleOverflowStateChange('11')}
            />
          </div>
        </div>
        {fifthLink && (
          <div className={getLinkWrapperClasses('5')}>
            <SharedButton
              className={linkClasses}
              variant={isOnDark ? 'primary-outlined' : 'primary'}
              onClick={handleSelectLink('5')}>
              {fifthLink.text}
            </SharedButton>
          </div>
        )}
...
export default FiveProductEditor;
