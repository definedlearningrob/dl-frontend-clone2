...
import defaultSlides from '@dc/components/Admin/Tasks/PresentationBuilder/SlidesList/Modal/defaultTemplates';
...
export const TemplatesTab = ({ closeModal }: Props) => {
  ...
  const handleAddNewSlide = async (template: keyof typeof defaultSlides) => {
    try {
      ...
      const slideTemplate = defaultSlides[template];
  ...
  return (
    <ul className='new-slide-content'>
      {Object.values(defaultSlides).map((slide: any) => (
        <li
          key={slide.template}
