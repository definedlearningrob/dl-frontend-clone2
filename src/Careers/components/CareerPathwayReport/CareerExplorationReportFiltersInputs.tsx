...
import {
  Option,
  useCareerExplorationReportFilters,
} from '@dc/components/CareerPathwayReport/useCareerExplorationReportFilters';
import { CAREER_EXPLORATION_REPORT_FILTERS } from '@dc/graphql/user/queries/careerExplorationReportFilters';
...
export const CareerExplorationReportFiltersInputs = ({ onStepChange = noop }: Props) => {
  const { t } = useTranslation();
  const isFullHD = useBreakpointUp({ breakpoint: 'xxxl' });
  const { filters, variablesForFilters, handleFilterChange } = useCareerExplorationReportFilters();

  const { data, loading } = useQuery(CAREER_EXPLORATION_REPORT_FILTERS, {
  ...
  useUpdateEffect(() => {
    ...
    const careerPathwayFilters = omitTypename(data.pathwayReportFilters);
  ...
  const teachersMap = getTeachersMap(data?.pathwayReportFilters?.schoolClasses.nodes);
    ...
    <div className='flex gap-sm xxxl:gap-base flex-1'>
      ...
      <div className='flex-1 flex flex-col gap-xs xxxl:gap-sm'>
        <ReportFiltersSelect
          ...
          name='entities'
          options={mapFilterOptions(data?.pathwayReportFilters.entities.nodes, 'uuid')}
        ...
        <ReportFiltersSelect
          ...
          name='gradeLevels'
          options={mapFilterOptions(data?.pathwayReportFilters.gradeLevels)}
        ...
        <ReportFiltersSelect
          ...
          name='users'
          options={mapFilterOptions(data?.pathwayReportFilters.users.nodes, 'uuid', 'fullName')}
        ...
        <Tooltip
          ...
          <ReportFiltersSelect
            ...
            name='schoolClasses'
            options={mapFilterOptions(data?.pathwayReportFilters.schoolClasses.nodes, 'uuid')}
