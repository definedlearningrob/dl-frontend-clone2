...
import {
  Option,
  useAssessmentReportFilters,
} from '@dc/components/AssessmentReport/useAssessmentReportFilters';
import { ASSESSMENT_REPORT_FILTERS } from '@dc/graphql/user/queries/assessmentReportFilters';
...
export const AssessmentReportFiltersInputs = ({ onStepChange = noop }: Props) => {
  const { t } = useTranslation();
  const isFullHD = useBreakpointUp({ breakpoint: 'xxxl' });
  const { filters, variablesForFilters, handleFilterChange } = useAssessmentReportFilters();

  const { data, loading } = useQuery(ASSESSMENT_REPORT_FILTERS, {
  ...
  useUpdateEffect(() => {
    ...
    const assessmentFilters = omitTypename(data.assessmentReportFilters);

    forEach(assessmentFilters, (filterData, filterName) => {
  ...
  const teachersMap = getTeachersMap(data?.assessmentReportFilters?.schoolClasses.nodes);
    ...
    <div className='flex gap-sm xxxl:gap-base flex-1'>
      ...
      <div className='flex-1 flex flex-col gap-xs xxxl:gap-sm'>
        <ReportFiltersSelect
          ...
          name='entities'
          options={mapFilterOptions(data?.assessmentReportFilters.entities.nodes, 'uuid')}
        ...
        <ReportFiltersSelect
          ...
          name='gradeLevels'
          options={mapFilterOptions(data?.assessmentReportFilters.gradeLevels)}
        ...
        <ReportFiltersSelect
          ...
          name='users'
          options={mapFilterOptions(data?.assessmentReportFilters.users.nodes, 'uuid', 'fullName')}
        ...
        <Tooltip
          ...
          <ReportFiltersSelect
            ...
            name='schoolClasses'
            options={mapFilterOptions(data?.assessmentReportFilters.schoolClasses.nodes, 'uuid')}
