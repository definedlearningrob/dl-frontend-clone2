...
import {
  ...
  counselorMidYearReportFormSchema,
  counselorMidYearReportFormUiSchema,
} from './CounselorMidYearReportSchemas';
import styles from './CounselorMidYearReport.module.sass';

type Props = {
  initialData: FormsInitialData;
};

export const CounselorMidYearReportForm = (props: Props) => {
  const { studentUuid } = useParams<{ studentUuid: string }>();
  const { t } = useTranslation();
  const formRef = useRef(null);
  const { saveForm, extraErrors, isSaving } = useFormSave<typeof counselorMidYearReportFormSchema>({
  ...
  const onSubmit = async () => {
    ...
    try {
      await submit({
        variables: {
          input: {
            type: COMMON_APP_FORM_TYPES.COUNSELOR_MIDYEAR_REPORT,
            studentUuid,
          },
        },
      });
  ...
  return (
    <div className={styles.formWrapper}>
      <JsonForm
        ...
        schema={counselorMidYearReportFormSchema}
        uiSchema={counselorMidYearReportFormUiSchema}
        onChange={(data) => {
